...
class HomeController extends Controller
{
    public function index(BorrowEligibilityService $borrowEligibility)
    {
        ...
        $announcements = PublicWebsiteContent::announcements()
            ->limit(6)
            ->get();

        $resources = PublicWebsiteContent::resources()
            ->limit(6)
            ->get();

        $facilities = PublicWebsiteContent::facilities()
            ->limit(6)
            ->get();
    ...
    private function getBooksQuery()
    {
        ...
        PublicWebsiteContent::onlyVisibleResources($books, 'r');
